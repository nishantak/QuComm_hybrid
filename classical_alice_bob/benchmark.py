...
def run_server_get_proc(host: str, port: int, tls_version: str, ciphers: str, cert: str, key: str, payload_bytes: int) -> subprocess.Popen:
...
def run_client(host: str, port: int, tls_version: str, ciphers: str, cafile: str, payload_bytes: int) -> None:
    python_exe = sys.executable
    cmd = [
        python_exe,
        "alice.py",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tls", choices=["1.3"], nargs="+", default=["1.3"])
...
def main() -> None:
    ...
    for tls_version in args.tls:
        ...
        for i in range(args.runs):
            server_proc = None
            try:
                append_log(f"Starting server for TLS {tls_version} run {i+1}/{args.runs}")
                ...
                if server_proc is not None:
                    try:
                        ...
                        try:
                            ...
                            pass
        all_runs.append({"tls_version": tls_version, "runs": run_results})
